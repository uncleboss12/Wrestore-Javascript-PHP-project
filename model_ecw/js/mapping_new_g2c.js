...
function initialize() {
    ...
    function doCropRotation(color_croprot, poly_opacity) {
        ...
        var cr_yes = 0;
        var cr_no = 0;
        //E: Iteration for each polygon (subbasin)
        for (var i = 0; i < subbasin_json.features.length; i++) {//EE: it gives: 130 times
            if (listofSubs.includes(subbasin_json.properties[i]["Subbasin"])) {
                ...
                //E: Set the previous polygons 'crop' into the basemap
                ...
                //E: Add a click-event to the map to show the previous info when the user makes click over the map
    ...
    function doCropRotation_markers() {
        ...
        for (var i = 0; i < sb_with_cr_only.properties.length; i++) {
            ...
            //EE: 'crop' connects to 'g2.php/checkBox_CropRotation(options)' to on/off by its check-button
            crop = geop;//geo;
            crop.setMap(basemap_1);
            cropArray.push(crop);
            crop1 = geop1;//geo;
        ...
        google.maps.event.addListener(basemap_1, 'zoom_changed', function() {
            ...
            for (i = 0; i < CropRotation_markers.length; i++) {
                ...
                CropRotation_markers[i].setVisible(zoom >= 20);
            }
        });
        google.maps.event.addListener(basemap_1, 'zoom_changed', function() {
            var zoom = basemap_1.getZoom();
            ...
            for (i = 0; i < CropRotation_markers1.length; i++) {
                ...
                CropRotation_markers1[i].setVisible(zoom < 20);
            }
        });
    ...
    // ++++++++++++++  This is used to parse out (definir,examinar) the long lats for all the polygons +++++++++++++++++//
  ...
  function toggleLayerNew(whichArray, mapName) {
      if (mapName.getMap()) {
          ...
          $.each(whichArray, function(index, value) {
              ...
              // alert ("L.11: " + whichArray);
              // alert ("L.11: " + JSON.stringify(whichArray));
              value.setMap(null);
          });
          ...
          $.each(whichArray, function(index, value) {
              // alert("This is value in else: "+value);
              // value.setMap(map1);
